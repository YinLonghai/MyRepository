		...
		inviteMember.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				inviteMembersDialog.setInviteMembersListener(new InviteMembersListener() {
					@Override
					public void getMembers(List<User> list) {System.out.println("user list size-"+list.size());
					...
					new Thread(new Runnable() {
						...
						public void run() {
							try {
								...
								if (imagePath == null) {
									chatGroup = new ChatGroupManager().createChatGroup(AppData.getUser(getContext())._id, sb.toString(), groupName.getText()
											.toString());
								}else {
									chatGroup = new ChatGroupManager().createChatGroup(AppData.getUser(getContext())._id, sb.toString(), groupName.getText()
											.toString(), StringUtils.getFileSuffix(imagePath), new File(imagePath));
									imagePath = null;
								}	
								if (chatGroup != null) {
...
//								handler.sendMessage(message);
	...
	public void setCreateChatGroupListener(
			CreateChatGroupListener createChatGroupListener) {
